...
export const ModeToggle: React.FC<ModeToggleProps> = ({
  ...
  return (
    <div className="flex flex-col items-center justify-center p-1 sm:p-1.5 rounded-2xl glass-panel-glow bg-black/70 border border-cyan-400/30 shadow-2xl backdrop-blur-2xl transition-all w-full">
      <div className="grid grid-cols-2 gap-1 sm:gap-1.5 w-full">
        {/* Mode 1: What I'm Holding */}
        <button
          onClick={() => handleSelect('HOLDING')}
          className={`relative flex items-center justify-center gap-1.5 sm:gap-2 py-2 sm:py-2.5 px-2 sm:px-4 min-h-[40px] rounded-xl font-space font-medium text-xs sm:text-sm tracking-wide transition-all duration-200 active:scale-95 ${
            currentMode === 'HOLDING'
      ...
      {currentMode === 'LOOKING_AT' && (
        <div className="flex items-center justify-center gap-1.5 mt-1.5 pt-1.5 border-t border-emerald-500/20 w-full animate-fadeIn">
          <div className="grid grid-cols-2 gap-1 w-full bg-black/40 p-0.5 rounded-xl border border-white/5">
            <button
              onClick={() => handleFramingSelect('FINGERS_FRAME')}
              className={`flex items-center justify-center gap-1 px-2 py-1 min-h-[30px] rounded-lg text-[10px] sm:text-[11px] font-mono transition-all active:scale-95 ${
                framingStyle === 'FINGERS_FRAME'
